...
krb5_error_code init_context(Kadmin *self, const char *keytab_path, const char *realm, char **error_msg) {
    ...
    if (ret) {
        *error_msg = strdup("Error starting keytab iteration");
        printf("Error starting keytab iteration: %s\n", *error_msg);
        goto cleanup;
    }
    ...
    if (ret) {
        *error_msg = strdup("Error getting next keytab entry");
        printf("Error getting next keytab entry: %s\n", *error_msg);
        goto cleanup;
    }
    ...
    if (ret) {
        *error_msg = strdup("Error unparsing principal name");
        printf("Error unparsing principal name: %s\n", *error_msg);
        goto cleanup;
    }
    ...
    if (ret) {
        *error_msg = strdup("Error initializing admin service");
        printf("Error initializing admin service: %s\n", *error_msg);
        goto cleanup;
    }
...
static int Kadmin_init(Kadmin *self, PyObject *args, PyObject *kwds) {
    ...
    if (ret) {
        PyErr_Format(PyExc_RuntimeError, "init_context failed: %s", error_msg);
        free(error_msg);
        return -1;
    }
...
static void Kadmin_dealloc(Kadmin* self) {
    if (self->client_name) free(self->client_name);
    if (self->server_handle) kadm5_destroy(self->server_handle);
    if (self->context) krb5_free_context(self->context);
