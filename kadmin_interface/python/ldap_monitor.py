...
class LDAPMonitor:
    ...
    def is_special_principal(self, principal):
        ...
        """
        return "/" in principal or "default" in principal or "admin" in principal or "bind" in principal or "testuser123" in principal
    ...
    def update_account_expires_in_ad(self, employee_id, kerberos_timestamp):
        """
        Update the accountExpires attribute in Active Directory.
        ...
        try:
            self.ad_ldap_conn.modify(
                dn,
                {'accountExpires': [(MODIFY_REPLACE, [ldap_timestamp])]}
            )
            if self.ad_ldap_conn.result['result'] == 0:
                pass
            else:
                self.logger.error("Failed to update accountExpires for %s: %s", dn, self.ad_ldap_conn.result['description'])
        except Exception as e:
            self.logger.error("Exception while updating accountExpires for %s: %s", dn, e)
    ...
    def update_ad_user(self, principal, password):
        ...
        """
        try:
            ad_dn, ad_sam_account_name = self.search_user_in_ad_by_employee_id(principal.employee_id)
        except Exception as e:
        ...
        for attempt in range(retries):
            if ad_dn:
                if os.path.exists(self.ad_config['keytab']):
                    try:
                    ...
                    self.logger.error("Keytab file does not exist.")
                break
            else:
                self.logger.info("User not found in AD, retrying in 10 seconds... (Attempt %d/%d)", attempt + 1, retries)
                time.sleep(10)
                ad_dn, ad_sam_account_name = self.search_user_in_ad_by_employee_id(principal.employee_id)

        if not ad_dn:
            self.logger.error("User not found in AD after %d attempts.", retries)
