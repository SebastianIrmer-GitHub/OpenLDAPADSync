...
# Define LDAP server URIs and credentials for source and target servers
source_server_uri = 'ldap://openldap.krb.uni-magdeburg.de:389'
source_bind_dn = 'cn=admin,dc=openldap,dc=krb,dc=uni-magdeburg,dc=de'
source_bind_password = 'Abc1234'

target_server_uri = 'ldap://kerberosad.uni-magdeburg.de:389'
target_bind_dn = "CN=Administrator,CN=Users,DC=kerberosad,DC=uni-magdeburg,DC=de"
target_bind_password = "Abc1234"


def get_all_ous(server_uri, bind_dn, bind_password):
    """
    Retrieve all organizational units (OUs) from an LDAP server.

    :param server_uri: URI of the LDAP server
    :param bind_dn: Bind DN for authentication
    :param bind_password: Password for authentication
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            search_base = 'dc=openldap,dc=krb,dc=uni-magdeburg,dc=de'
...
def get_all_target_ous(server_uri, bind_dn, bind_password):
    """
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            search_base = 'dc=kerberosad,dc=uni-magdeburg,dc=de'
...
def move_ou(server_uri, bind_dn, bind_password, old_dn, new_dn, ou_name):
    """
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            if conn.modify_dn(old_dn, ou_name, new_superior=new_dn):
                # print(f"OU moved from {old_dn} to {new_dn}.")
                pass
            else:
                pass
                # print(f"Failed to move OU from {old_dn} to {new_dn}.")
    except Exception as e:
...
def ou_exist_under_base(server_uri, bind_dn, bind_password, base_dn, ou_id):
    """
...
def create_ou_if_not_exists(server_uri, bind_dn, bind_password, ou_name, distinguished_name, ou_id, ou_parent_id,
                            target_base="dc=kerberosad,dc=uni-magdeburg,dc=de"):
    """
    ...
    :param target_base: Base distinguished name in the target LDAP
    """
    server = Server(server_uri, get_info=ALL)
    new_target_dn = distinguished_name.replace("dc=openldap,dc=krb,dc=uni-magdeburg,dc=de", target_base)
    base_dn = f"{target_base}"
    new_target_dn = convert_specific_parts_to_lowercase(new_target_dn)
    try:
        with (Connection(server, bind_dn, bind_password, auto_bind=True) as conn):
            ou_exists, existing_target_dn = ou_exist_under_base(server_uri, bind_dn, bind_password, base_dn, ou_id)
            if ou_exists:
                existing_target_dn = convert_specific_parts_to_lowercase(existing_target_dn)
                if new_target_dn == existing_target_dn:
                    # print(f"OU {new_target_dn} already exists in {server_uri} in the correct position.")
                    pass
                else:
                    move_ou(server_uri, bind_dn, bind_password, existing_target_dn, new_target_dn, ou_name)
            else:
                attributes = {
                ...
                if conn.result['result'] == 0:
                    # (f"OU {new_target_dn} added to {server_uri}.")
                    pass
                else:
                    print(f"Failed to add OU {new_target_dn} to {server_uri}. Error: {conn.result}")
    except Exception as e:
...
def check_if_user_moved(server_uri, bind_dn, bind_password, employee_id):
    """
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            if employee_id:
                ...
                if conn.search(search_base='dc=openldap,dc=krb,dc=uni-magdeburg,dc=de', search_filter=search_filter,
                               search_scope=SUBTREE):
                    if len(conn.entries) > 0:
                        return True, conn.entries[0].entry_dn.replace("dc=openldap,dc=krb,dc=uni-magdeburg,dc=de",
                                                                      "dc=kerberosad,dc=uni-magdeburg,dc=de")
            return False, None
    except Exception as e:
        traceback.print_exc()
...
def check_if_ou_moved(server_uri, bind_dn, bind_password, ou_dn):
    """
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            ...
            if conn.search(search_base='dc=dc=openldap,dc=krb,dc=uni-magdeburg,dc=de', search_filter=search_filter,
                           search_scope=SUBTREE):
                if len(conn.entries) > 0:
                    return True, conn.entries[0].entry_dn.replace("dc=openldap,dc=krb,dc=uni-magdeburg,dc=de",
                                                                  "dc=kerberosad,dc=uni-magdeburg,dc=de")
...
def check_and_move_child_elements(server_uri, bind_dn, bind_password, dn):
    """
    ...
    try:
        with Connection(server, bind_dn, bind_password, auto_bind=True) as conn:
            search_filter = f'(|(objectClass=organizationalUnit)(objectClass=person))'
            ...
            if conn.search(dn, search_filter, attributes=attributes, search_scope=SUBTREE):
                for entry in conn.entries:
                    ...
                    if 'employeeID' in entry:
                        ...
                        user_moved, new_dn = check_if_user_moved(source_server_uri, source_bind_dn,
                                                                 source_bind_password, employee_id)
                        if user_moved:
                            move_user(server_uri, bind_dn, bind_password, entry_dn, new_dn)

                    else:
                        ou_moved, new_dn = check_if_ou_moved(source_server_uri, source_bind_dn, source_bind_password,
                                                             entry_dn)
                        if ou_moved:
                            move_ou(server_uri, bind_dn, bind_password, entry_dn, new_dn, entry.ou.value)
    except Exception as e:
        traceback.print_exc()
        print("LDAP error during child element check:", str(e))


def synchronize_ous(source_server_uri, source_bind_dn, source_bind_password, target_server_uri, target_bind_dn,
                    target_bind_password):
    """
    Synchronize OUs from the source LDAP server to the target LDAP server.

    :param source_server_uri: URI of the source LDAP server
    :param source_bind_dn: Bind DN for the source LDAP
    :param source_bind_password: Password for the source LDAP
    :param target_server_uri: URI of the target LDAP server
    :param target_bind_dn: Bind DN for the target LDAP
    :param target_bind_password: Password for the target LDAP
    """
    source_ous = get_all_ous(source_server_uri, source_bind_dn, source_bind_password)

    # Synchronize OUs from source to target
    for ou_name, dn, ou_id, ou_parent_id in source_ous:
        create_ou_if_not_exists(target_server_uri, target_bind_dn, target_bind_password, ou_name, dn, ou_id,
                                ou_parent_id)


if __name__ == "__main__":
    print("Starting OU creation and modify.")
    synchronize_ous(source_server_uri, source_bind_dn, source_bind_password, target_server_uri, target_bind_dn,
                    target_bind_password)
    print("OUs created...")
    print("OUs modified...")
